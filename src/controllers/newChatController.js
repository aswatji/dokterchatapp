...
const chatController = {
  ...
  createOrGetChat: async (req, res) => {
    try {
      ...
      const usersCheck = await db.query('SELECT uid FROM users WHERE uid IN ($1, $2)', [user1Id, user2Id]);
  ...
  getChatById: async (req, res) => {
    try {
      ...
      console.log(`✅ Chat found between ${chat.user1_name} and ${chat.user2_name}`);
  ...
  deleteChat: async (req, res) => {
    try {
      ...
      try {
        ...
        await client.query('DELETE FROM last_messages WHERE chat_id = $1', [id]);
