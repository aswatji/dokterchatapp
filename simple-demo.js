...
app.listen(PORT, () => {
  ...
  console.log(
    `📊 Demo data: ${users.length} users, ${chats.length} chats, ${messages.length} messages`
  );
  ...
  console.log(
    `curl -X GET "http://localhost:${PORT}/api/chats?userId=${users[0].uid}"`
  );
  console.log(
    `curl -X GET "http://localhost:${PORT}/api/chats/${chats[0].chat_id}/messages"`
  );
  console.log("");
});
